---
title: "StratoSharkとは？ ― Wiresharkのシステムコール解析版"
---

# StratoSharkとは？

## StratoShark徹底解説：eBPFで実現するシステム可観測性

### StratoSharkとは何か？

**StratoShark**は、Wiresharkの作者Gerald Combs氏とSysdig/Falco創設者のLoris Degioanni氏が協働開発した**システムコール・ログ解析ツール**です。2025年1月に正式リリースされ、Wiresharkと同じコードベースに統合されながら、まったく異なる目的で設計されています。

:::message alert
**重要：StratoSharkはネットワークパケットを解析しません**

Wiresharkがネットワークパケットを解析するのに対し、StratoSharkは**システムコールとログメッセージ**を解析します。ネットワークの内部ではなく、システムの内部を見るツールです。
:::
...
|------|-----------|-------------|
| **解析対象** | ネットワークパケット | システムコール・ログ |
| **データソース** | libpcap（ネットワークインターフェース） | eBPF（Linuxカーネル）+ libscap/libsinsp |
| **キャプチャ方式** | pcap（Packet Capture） | scap（System Capture） |
| **主な用途** | ネットワークプロトコル解析 | システム・アプリケーションレベル解析 |
| **何が見える？** | TCP/IP、HTTP、DNS等のパケット | open()、read()、write()等のシステムコール |

**比喩で理解する**：
- **Wireshark** = 道路の監視カメラ（車の動きを外から観察）
- **StratoShark** = 車内のドライブレコーダー（運転手の操作を内部から記録）

### 技術的コアコンセプト

StratoSharkは3つの強力なライブラリに基づいています：

#### 1. **libscap** - System Capture Library
```
役割: カーネルからシステムコールイベントを低レベルキャプチャ
機能:
  ✓ eBPFドライバー経由でのライブキャプチャ
  ✓ .scapファイル（トレースファイル）の読み書き
  ✓ OSステート情報の取得
```

#### 2. **libsinsp** - System Inspection Library
```
役割: 生のシステムコールデータをリッチなコンテキスト情報に変換
機能:
  ✓ プロセスメタデータの付加（PID、コマンドライン等）
  ✓ ファイルディスクリプタの追跡
  ✓ コンテナ・Kubernetes情報の紐付け
  ✓ 高度なフィルタリングエンジン
```

#### 3. **Falco Plugins**
```
役割: AWSやクラウドログソースとの統合
機能:
  ✓ AWS CloudTrailログの解析
  ✓ S3/SQS/SNSからのデータ取得
  ✓ カスタムデータソースの拡張
...
- どのファイルが開かれたか？
- どのシステムコールが失敗したか？
- コンテナ間でどのようにファイルディスクリプタが操作されたか？
- 暗号化されたTLS通信の中身（アプリ層で見れば平文）

# Kubernetes環境で何が起きているか？
- CrashLoopBackOffの真の原因
- Podが起動しない理由
- なぜコンテナが突然終了したか？
```

**StratoSharkで見えるようになる世界**：
```bash
# 実際のシステムコールレベルで追跡可能
execve("/bin/sh", ...)  # プロセス起動
open("/etc/config", O_RDONLY) = -1 ENOENT  # 設定ファイルが見つからない
connect(3, {sa_family=AF_INET, ...}) = -1 ETIMEDOUT  # 接続タイムアウト
write(4, "API_KEY=secret...", 128)  # 機密情報の漏洩検知
```

### 実用例：StratoSharkの威力

#### 例1：CrashLoopBackOffの解決
```bash
# 従来の方法：ログを見ても原因不明
kubectl logs my-pod
# => Error: exit code 1
...
# StratoSharkで解析
stratoshark capture --pod my-pod
# => 発見：open("/app/config/database.yaml") = -1 EACCES (Permission denied)
# => 真因：ConfigMapがマウントされていなかった
```

#### 例2：HTTP/HTTPS通信の完全解析
```bash
# Wiresharkでは：TLSで暗号化されていて中身が見えない
# StratoSharkでは：アプリケーション層のシステムコール（read/write）で平文を取得可能

# HTTP/HTTPSトラフィックをシステムコールレベルで解析
stratoshark capture --filter "fd.type=ipv4" --pod api-server
# => 実際に送受信されたHTTPリクエスト/レスポンスの中身が見える
```

#### 例3：ファイルディスクリプタのリーク検出
```bash
# プロセスがファイルディスクリプタを開きすぎて枯渇する問題
stratoshark analyze trace.scap --show-fd-leaks
# => nginx worker process: 1024 file descriptors opened but never closed
```
...
| ユースケース | 説明 | 従来の課題 |
|------------|------|-----------|
| **クラウドセキュリティ** | AWS CloudTrailログをWireshark UIで解析 | JSONログを手作業で検索 |
| **コンテナデバッグ** | Podが起動失敗する原因を特定 | ログに情報が残らない |
| **パフォーマンス調査** | システムコールレベルでのボトルネック特定 | APMツールでは粒度が粗い |
| **セキュリティ監査** | ファイルアクセス・ネットワーク接続の完全追跡 | Auditログは設定が複雑 |
| **インシデント対応** | Falcoアラートと連携して詳細調査 | アラートだけでは原因不明 |
...
- ✅ **SREエンジニア** - Kubernetes環境でのトラブルシュート効率化
- ✅ **セキュリティエンジニア** - ランタイムセキュリティの可視化
- ✅ **プラットフォームエンジニア** - コンテナプラットフォームの深い理解
- ✅ **Falcoユーザー** - アラートの詳細調査
- ✅ **Wiresharkユーザー** - システムレベル解析への拡張

:::message
**前提知識**
- Linuxの基本的なシステムコール知識（open、read、write等）
- Kubernetesの基礎（Pod、Namespace等）
- Wiresharkの基本的な使い方（推奨、必須ではない）
:::
